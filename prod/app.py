...
RUTA_MEJOR_MODELO = ROOT_DIR / "dev" / "modelo.pth"

# Fallbacks para correr la app desde distintas carpetas durante pruebas locales.
if not RUTA_MEJOR_MODELO.exists():
    fallback_paths = [
        Path(__file__).resolve().parent / "best_model.pt",
        Path.cwd() / "best_model.pt",
        Path.cwd() / "dev" / "modelo.pth",
    ]
    RUTA_MEJOR_MODELO = next((p for p in fallback_paths if p.exists()), RUTA_MEJOR_MODELO)
